...
function updateAlbum($db) {
    ...
    try {
        ...
        // Update album
        $query = "UPDATE albums SET title = :title, year = :year, category = :category, 
                    cover_image = :cover_image, description = :description, updated_at = NOW()
                  WHERE id = :id";
        ...
        if ($stmt->execute()) {
            ...
            $deleteTracksQuery = "DELETE FROM tracks WHERE album_id = ?";
...
function deleteAlbum($db) {
    ...
    if (!$albumId) {
        ...
        echo json_encode(['message' => 'Album ID is required']);
        return;
    }
    ...
    try {
        ...
        $deleteStmt = $db->prepare($deleteAlbumQuery);
        $deleteStmt->execute([$albumId]);
        
        echo json_encode(['message' => 'Album deleted successfully']);
    } catch (Exception $e) {
        http_response_code(500);
        echo json_encode(['message' => 'Album deletion failed: ' . $e->getMessage()]);
    }
}
?>
